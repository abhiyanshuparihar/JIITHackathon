...
class FreshQRScannerScreen extends StatefulWidget {
  @override
  _FreshQRScannerScreenState createState() => _FreshQRScannerScreenState();
}

class _FreshQRScannerScreenState extends State<FreshQRScannerScreen>
  ...
  void _onBarcodeDetected(BarcodeCapture capture) {
    ...
    if (barcodes.isNotEmpty && barcodes.first.rawValue != null) {
      ...
      _handleDetectionResult(code);
    }
  }

  void _handleDetectionResult(String code) async {
    scannerController.stop();

    final bool isUpi = code.startsWith('upi://pay');
    if (isUpi) {
      _showMessage('UPI QR code detected!', MessageType.success);
    } else {
      _showMessage('Invalid QR code format', MessageType.warning);
    }

    await Future.delayed(Duration(milliseconds: 800));
    _handleNavigation(code);
  }

  Future<void> _handleNavigation(String code) async {
    final firestore = FirebaseFirestore.instance;
    const String collectionName = 'stores';

    try {
      _showMessage('Checking database...', MessageType.info);

      final querySnapshot = await firestore
          .collection(collectionName)
          .where('upiUri', isEqualTo: code)
          .get();

      if (querySnapshot.docs.isNotEmpty) {
        _showMessage('Store found! Redirecting...', MessageType.success);
        ...
        if (mounted) {
          await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => PayCreditWithUpiUriPage(upiUri: code),
            ),
          );
          _resetScanner();
        }
      } else {
        _showMessage('Store not registered in our system', MessageType.error);
        await Future.delayed(const Duration(seconds: 2));
        _resetScanner();
      }
    } catch (e) {
      _showMessage('Database error: ${e.toString()}', MessageType.error);
      await Future.delayed(const Duration(seconds: 2));
      _resetScanner();
    }
  }
  ...
  void _resetScanner() async {
    ...
    try {
      ...
      _showMessage('Failed to resume scanner', MessageType.error);
    }
  }
  ...
  Future<void> _pickFromGallery() async {
    ...
    try {
      ...
      _showMessage('Failed to analyze image', MessageType.error);
      _resetScanner();
    }
  }
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              if (_messageType == MessageType.error || _messageType == MessageType.warning)
                GestureDetector(
                  onTap: _hideMessage,
          ...
          child: SizedBox(
            ...
            child: Stack(
              children: [
                ...
                ...List.generate(4, (index) => _buildAnimatedCorner(index, scanAreaSize)),
                Container(
                  decoration: BoxDecoration(
  ...
  Widget _buildHeaderButton(IconData icon, VoidCallback onTap, {bool isActive = false}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isActive ? Colors.amber.withOpacity(0.2) : Colors.white70,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isActive ? Colors.amber.withOpacity(0.5) : Colors.white.withOpacity(0.2),
            width: 1,
          ),
        ),
        child: Icon(
          icon,
          color: isActive ? Colors.amber : MyColors.blueColor,
          size: 22,
        ),
      ),
    );
        ...
        AnimatedBuilder(
          ...
          builder: (context, child) {
            return Transform.scale(
              scale: isCameraActive && !_isProcessing ? _cornerAnimation.value : 1.0,
                ...
                child: Container(
                  ...
                  child: _isProcessing
                    ...
                    child: CircularProgressIndicator(
                      ...
                      : Icon(
                    isCameraActive ? Icons.qr_code_scanner_rounded : Icons.play_arrow_rounded,
                    color: Colors.white,
                    size: 36,
                  ),
                ),
