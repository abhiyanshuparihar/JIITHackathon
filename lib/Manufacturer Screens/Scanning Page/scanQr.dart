...
class ProductQRScannerScreen extends StatefulWidget{
  const ProductQRScannerScreen({super.key});
  @override
  _ProductQRScannerScreenState createState() => _ProductQRScannerScreenState();
}

class _ProductQRScannerScreenState extends State<ProductQRScannerScreen>
  ...
  void _onBarcodeDetected(BarcodeCapture capture) {
    ...
    if (barcodes.isNotEmpty && barcodes.first.rawValue != null) {
      ...
      _handleDetectionResult(productHash);
    }
  }

  void _handleDetectionResult(String productHash) async {
    ...
    try {
      ...
      // 2. Search in productQrs collection
      final productQuerySnapshot = await firestore
      ...
      if (productQuerySnapshot.docs.isNotEmpty) {
        // 3. Check duplicacy in productTrackRecord
        final productData = productQuerySnapshot.docs.first.data();
        final List<dynamic> trackRecords = productData['productTrackRecord'] ?? [];

        bool isDuplicate = false;
        for (var record in trackRecords) {
          if (record is Map<String, dynamic> && record['name'] == userRole) {
            isDuplicate = true;
            break;
          }
        }
        if (isDuplicate) {
          _showMessage('Ethereum says tempering not allowed', MessageType.error);
          await Future.delayed(const Duration(seconds: 2));
          _resetScanner();
          return;
        }
        _showMessage('Product found!', MessageType.success);
        await Future.delayed(Duration(milliseconds: 800));
        if (mounted){
          await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ProductFormPage(productHash: productHash),
            ),
          );
          _resetScanner();
        }
      ...
      // 4. Else: Search in parentQrs
      final parentQuerySnapshot = await firestore
      ...
      if (parentQuerySnapshot.docs.isNotEmpty) {
        _showMessage('Parent product found!', MessageType.success);
        await Future.delayed(Duration(milliseconds: 800));
        if (mounted) {
          await Navigator.push(
            context,
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const Text(
              'Looking for registered product hash...',
              style: TextStyle(
