...
class WareHousingQRScannerScreen extends StatefulWidget{
  const WareHousingQRScannerScreen ({super.key});
  @override
  _WareHousingQRScannerScreenState createState() => _WareHousingQRScannerScreenState();
}

class _WareHousingQRScannerScreenState extends State<WareHousingQRScannerScreen>
  ...
  void _handleWarehousingScan(String productHash) async {
    ...
    try {
      ...
      // 1. Fetch current user details for the track record
      final userData = await _fetchCurrentUserDetails(currentUser.uid);
      final String userRole = userData['role'] ?? 'Manufacturer';

      if (userData.isEmpty) {
        throw Exception("Failed to fetch user details for track record.");
      }

      // 2. Check in parentQrs collection
      ...
      if (parentQuerySnapshot.docs.isNotEmpty) {
        ...
        // 3. Check in productQrs collection (single item)
        ...
        if (productQuerySnapshot.docs.isNotEmpty) {
          // --- Logic for Single Product Hash Scan ---
  ...
  Future<void> _processBatchUpdate({
    ...
    _showMessage('Verifying warehousing status...', MessageType.info);

    // Check for duplicate warehousing record
    ...
    for (var doc in productQrsSnapshot.docs){
      ...
      // Look for any map in the trackRecord array that contains the key 'warehousing'
      final hasWarehousingRecord = trackRecord.any((record) =>
      record is Map &&
          record.containsKey('warehousing')
      );
      if (hasWarehousingRecord) {
        // If any linked product already has a warehousing record, show error and stop
    ...
    // --- Proceed with Warehousing Update ---

    // Simulate user confirmation on the next screen (replace with Navigator.push)
    final bool confirmed = await _simulateWarehousingProcess(context, productHashes.first);

    if (!confirmed) {
      _showMessage('Warehousing update cancelled.', MessageType.info);
      _resetScanner();
      return;
    }

    _showMessage('Saving warehousing details...', MessageType.info);

    // FIX: Use a client-side Timestamp object instead of FieldValue.serverTimestamp()
    // for nested fields in arrayUnion.
    final Timestamp clientTimestamp = Timestamp.fromDate(DateTime.now());

    // Create the new warehousing map
    final Map<String, dynamic> warehousingMap = {
      'warehousing': {
        'timestamp': clientTimestamp, // CORRECTED FIX
        'address': userData['address'],
        'warehouse_name': userData['manufacturer_name'], // Using manufacturer_name as warehouse name
        'email': userData['email'],
        'phone': userData['phone'],
        'role': 'warehousing', // Ensure role is specific to the action
        'uid': userData['uid'],
      }
    };

    const String newStatus = 'Ware Housing Details added';
    ...
    for (var doc in productQrsSnapshot.docs) {
      ...
      batch.update(docRef, {
        ...
        'status': newStatus,
        // This serverTimestamp is fine because it's at the top level of the update map
    ...
    if (parentDocId != null) {
      ...
      batch.update(parentRef, {
        'wareHousingBy':FirebaseAuth.instance.currentUser!.uid,
        'status': newStatus,
        'updatedAt': FieldValue.serverTimestamp(),
      });
    }
    ...
    _showMessage('Warehousing details saved successfully for $count document(s)!', MessageType.success);
    await Future.delayed(const Duration(seconds: 2));
    _resetScanner();
  }
  ...
  void _resetScanner() async {
    ...
    try {
      ...
      setState(() {
        ...
        isCameraActive = true;
        _isProcessing = false;
        _hideMessage(); // Ensure any residual message is cleared
      });
    } catch (e) {
      setState(() {
        _isProcessing = false;
  ...
  // --- UI Build Methods (Same as before) ---
  ...
  Widget _buildHeaderButton(IconData icon, VoidCallback onTap,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(
          icon,
          color: isActive ? MyColors.blueColor : MyColors.blueColor,
          size: 22,
        ),
      ),
    );
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          _isProcessing
              ? 'Processing...'
              : (isCameraActive ? 'Scanning...' : 'Start Scan'),
          style: const TextStyle(
            color: Colors.white,
            fontSize: 13,
            fontWeight: FontWeight.w600,
