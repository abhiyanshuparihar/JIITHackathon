...
import 'package:iiitmprojectbakaya/WholeSalers%20Screen/Scanning%20and%20Saving%20Data/scanningAndSavingData.dart';
...
class _WholesalerHomeScreenState extends State<WholesalerHomeScreen>
  ...
  // Animation Controllers same as your WareHousing page
  ...
  String _wholesalerName = 'Wholesaler';
  ...
  Future<void> _fetchWholesalerName() async {
    if (_currentUser != null) {
      try {
        ...
        if (userDoc.exists) {
          setState(() {
            _wholesalerName = (userDoc.data() as Map<String, dynamic>?)?['firstName'] ?? 'Wholesaler';
          });
        }
      } catch (e) {
  ...
  Widget _buildActionButton(IconData icon, String label, LinearGradient gradient, Color accentColor) {
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => WholesalerQRScannerScreen()),
        );
      },
      child: Column(
        children: [
  ...
  Widget _buildInventoryStream() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Wholesaler Inventory', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: MyColors.blueColor)),
        SizedBox(height: 16),
        StreamBuilder<QuerySnapshot>(
          stream: _firestore
              .collection('parentQrs')
              .where('wholesalerUid', isEqualTo: _currentUser!.uid) // ONLY CHANGE HERE FOR WHLESALER
