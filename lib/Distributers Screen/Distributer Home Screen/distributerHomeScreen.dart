...
import 'package:iiitmprojectbakaya/Distributers%20Screen/Scanning%20and%20Saving%20Page/scanningAndSavingPage.dart';
...
class Distributerhomescreen extends StatefulWidget {
  @override
  _DistributerhomescreenState createState() => _DistributerhomescreenState();
}
class _DistributerhomescreenState extends State<Distributerhomescreen>
  ...
  String _distributerName = 'Distributer';
  ...
  void _initAnimations() {
    // Exactly same as WareHousing page initialization ...
    ...
    _scaleController = AnimationController(duration: Duration(milliseconds: 800), vsync: this);
    _rotateController = AnimationController(duration: Duration(milliseconds: 2000), vsync: this);
    _drawerController = AnimationController(duration: Duration(milliseconds: 300), vsync: this);
    ...
    _drawerSlideAnimation = Tween<Offset>(begin: Offset(-1.0, 0), end: Offset.zero)
  ...
  Future<void> _fetchWholesalerName() async {
    if (_currentUser != null) {
      try {
        DocumentSnapshot userDoc = await _firestore.collection('users').doc(_currentUser!.uid).get();
        if (userDoc.exists) {
          setState(() {
            _distributerName = (userDoc.data() as Map<String, dynamic>?)?['firstName'] ?? 'Wholesaler';
          });
        }
      } catch (e) {
        print('Error fetching wholesaler name: $e');
      }
    }
  }
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      SizedBox(height: 10),
                      SlideTransition(position: _slideAnimation, child: _build3DVisualSection()),
                      SizedBox(height: 30),
                      SlideTransition(position: _slideAnimation, child: _buildQuickActions()),
                      SizedBox(height: 30),
                      SlideTransition(position: _slideAnimation, child: _buildInventoryStream()),
                      SizedBox(height: 30),
                    ],
                  ),
                ),
        ...
        children: [
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Distributer",
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: MyColors.blueColor),
                    ),
                  ],
                ),
              ),
          ...
          Container(
              ...
              decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [
                BoxShadow(color: Colors.blue.withOpacity(0.2), blurRadius: 10, offset: Offset(0, 2))
              ]),
              child: IconButton(
                  onPressed: () {
  ...
  Widget _build3DVisualSection() {
    // Similar as WareHousing (_build3DVisualSection) but you can customize text/icons for wholesaler
    return Container(
      height: 280,
      decoration: BoxDecoration(
        gradient: MyGradients.blueGradient,
        borderRadius: BorderRadius.circular(28),
        boxShadow: [BoxShadow(color: MyColors.blueColor.withOpacity(0.3), blurRadius: 30, offset: Offset(0, 15))],
      ),
      child: Stack(
        ...
        children: [
          ...
          Positioned(
            ...
            child: Container(
              height: 220,
              decoration: BoxDecoration(
                gradient: LinearGradient(colors: [Colors.white.withOpacity(0.1), Colors.white.withOpacity(0.1)]),
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        builder: (context, child) {
                          return Transform(
                            ...
                            child: Container(
                              ...
                              child: Icon(Icons.store_mall_directory, color: Colors.white, size: 32),
                            ),
                          );
                        },
                      ),
                    ...
                    Row(children: [
                      ...
                      SizedBox(width: 24),
                      _build3DFeatureIcon(Icons.local_shipping, 'Distribute'),
                      SizedBox(width: 24),
                      _build3DFeatureIcon(Icons.assignment_turned_in, 'Track'),
                    ]),
                  ],
                ),
              ),
          ...
          _buildFloatingPaymentIcon(Icons.qr_code_scanner, Offset(-20, 200), Colors.orange),
          _buildFloatingPaymentIcon(Icons.local_shipping, Offset(300, 50), Colors.green),
          _buildFloatingPaymentIcon(Icons.store, Offset(320, 220), Colors.blue),
        ],
      ),
    );
  }
      ...
      builder: (context, child) {
        ...
        return Positioned(
          ...
          child: Container(
            width: sizes[index],
            height: sizes[index],
            decoration: BoxDecoration(gradient: RadialGradient(colors: [colors[index], Colors.transparent]), shape: BoxShape.circle),
          ),
        );
      },
    );
  ...
  Widget _buildQuickActions() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Quick Actions', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: MyColors.blueColor)),
        SizedBox(height: 16),
        Container(
          ...
          child: Row(
            ...
            children: [
              _buildActionButton(Icons.qr_code_scanner, 'Scan QR', MyGradients.blueGradient, Colors.orange),
            ],
          ),
        ),
      ],
  ...
  Widget _buildActionButton(IconData icon, String label, LinearGradient gradient, Color accentColor) {
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => DistributerQRScannerScreen()),
        );
      },
      child: Column(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: gradient,
              borderRadius: BorderRadius.circular(16),
              boxShadow: [BoxShadow(color: accentColor.withOpacity(0.3), blurRadius: 12, offset: Offset(0, 6))],
            ),
  ...
  Widget _buildInventoryStream() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Distributer Inventory', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: MyColors.blueColor)),
        SizedBox(height: 16),
        StreamBuilder<QuerySnapshot>(
          stream: _firestore
              .collection('parentQrs')
              .where('distributerUid', isEqualTo: _currentUser!.uid) // ONLY CHANGE HERE FOR WHLESALER
              .orderBy('quantity', descending: true)
              .limit(_pageSize)
              .snapshots(),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) return _buildLoadingState();
            if (snapshot.hasError) return _buildErrorState('Error loading inventory: ${snapshot.error}');
            if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return _buildEmptyState();

            return FutureBuilder<List<ParentProductData>>(
              future: _processDocuments(snapshot.data!.docs),
              builder: (context, futureSnapshot) {
                if (futureSnapshot.connectionState == ConnectionState.waiting) return _buildLoadingState();

                if (futureSnapshot.hasError) return _buildErrorState('Error processing data: ${futureSnapshot.error}');

                if (!futureSnapshot.hasData || futureSnapshot.data!.isEmpty) return _buildEmptyState();

                return Column(children: futureSnapshot.data!.map((data) => _buildInventoryCard(data)).toList());
              },
            );
          },
        ),
...
// _processDocuments, _buildLoadingState, _buildErrorState, _buildEmptyState, _buildInventoryCard, _buildDetailRow
// These methods remain the same as WareHousing screen -- you can reuse exactly

// ...(reuse the functions exactly from WareHousing page without changes)
}
