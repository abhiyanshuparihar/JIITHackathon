...
class DistributerQRScannerScreen extends StatefulWidget {
  const DistributerQRScannerScreen({super.key});
  @override
  _DistributerQRScannerScreenState createState() => _DistributerQRScannerScreenState();
}

class _DistributerQRScannerScreenState extends State<DistributerQRScannerScreen> with TickerProviderStateMixin {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        return {
          ...
          'role': userDoc.data()?['role'] ?? 'Distributer',
          'uid': userId,
        };
      }
    } catch (e) {
  ...
  Future<void> _processBatchUpdate({
    ...
    _showMessage('Verifying Distributer status...', MessageType.info);

    // Check for existing distributer records in trackRecord
    ...
    for (var doc in productQrsSnapshot.docs) {
      ...
      record is Map && record.containsKey('distributer'));
    ...
    if (!confirmed) {
      _showMessage('Distributer update cancelled.', MessageType.info);
      _resetScanner();
      return;
    }

    _showMessage('Saving distributer details...', MessageType.info);
    ...
    const String newStatus = 'Distributer Details added';
    ...
    if (parentDocId != null) {
      ...
      batch.update(parentRef, {
        'distributerUid': _auth.currentUser!.uid,
        'status': newStatus,
        'updatedAt': FieldValue.serverTimestamp(),
      });
    }
    ...
    _showMessage('Distributer details saved successfully for $count document(s)!', MessageType.success);
    await Future.delayed(const Duration(seconds: 2));
    _resetScanner();
  }
